...
- `--request` is REPEATABLE — a batch of probes answers in one run,
  ...
  is answered with the PREDICTED refusal (400/414/501), which is
  itself the dry-run's answer.
...
- `--client <addr>` supplies the one decision input an offline run may
  ...
  `$remote_addr` renders UNRESOLVED by name.
...
- Exit code is `-t`'s: a config that fails `-t` prints the same
  `[emerg]` lines and predicts NOTHING (a prediction over a config
  the server would not load would be fiction). The dry-run knobs
  require `-t`; anywhere else they are a named CLI error.
...
hot path leaves it empty and allocation-free) — never a parallel
reimplementation. Each candidate carries a STABLE code plus operator
prose from ONE table (`dryrun.why_prose`, pinned by
`tests/dryrun/why_table.lu`; ws-docs reuse the wording from there):
...
- `refuse` — a NAMED refusal with its predicted status: parse-stage
  ...
  proxy-over-TLS 502 (the ws05 residue, predicted exactly as the live
  server answers it).
...
404 decision gets its explanation, by name.

## The JSON twin (schema 1)

`--format json` — a public tooling surface from day one (the ws08/ws12
convention), so it is versioned before anyone depends on it, hand-built
...
- **proxy**: predict the outbound upstream target for `GET /rewrite/a`
  ...
  RECORDED is exactly the predicted target.

Purity is enforced the same falsifiable way: the whole offline
pipeline runs on the interpreter lane (`tests/dryrun/purity_offline.lu`),
which has no sockets and no fs by design — any io on the called path
turns the lane red. The probe count is ratcheted
