...
again, so a backend that moves is a 502 until someone reloads. That
...
it, the deltas from nginx's `resolver`, and the two upstream asks the
design had to route around.
...
delay although it needed no name at all. In the after row the static
request answers in one millisecond while the resolver holds its
answer for eight hundred. The proxied request itself waits for the
answer, and the second request for the same name makes no round
trip (the DNS server's record file holds one query).
...
which is what the new `resolver` module does, spawn-free, in three
halves:
...
2. The cache: host → address set + expiry + a round-robin cursor.
   `valid=` overrides the record TTL as nginx's parameter does. A
   0-TTL answer is held one second, and so is a failure (both named
   below).
...
parameters nginx accepts but lobo cannot honour are refused with
their reason (`ipv4=off`: A records only at this pin; `status_zone=`:
no resolver status zone, the counters are at `/metrics`). The full
rows are in `docs/directives.md`.
...
loop, which is the stall this page measures. LOBO-L013 fires when a
resolver IS in effect for a static `proxy_pass` name, because that is
the
...
own, and the table above lists them.
...
- Events (ws09 vocabulary, appended, nothing renamed; docs/
  ...
  emission seam.
- Metric: `lobo_upstream_resolutions_total{result="ok"|"failed"}`, a
  fixed two-value label set, both series always present; the reason
  lives in the event, and the ws12 cardinality fence keeps it out of
  a label.
...
stays as it is; the seam is drawn at the socket.
